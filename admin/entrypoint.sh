...
# ensure there is a logs archive directory
mkdir -p /app/logs/archive
chmod 2770 /app/logs/archive
chown tok:web /app/logs/archive
...
# and also set up some additional configuration that they'll need
  ...
  if [[ -d "/app/config/host_ssh_keys" ]]; then
    cp /app/config/host_ssh_keys/* /etc/ssh/
  fi
  # Create user accounts based on the ssh keys present
  for f in /app/config/users/*;
  do
    [[ -e $f ]] || { echo "ERROR: No ssh keys found. Can't create users"; exit 1; }
    username=${f##*/}
    useradd -s /bin/bash -g web -m -K UMASK=002 "$username"
    usermod -p '*' "$username"
    tee /home/"$username"/.ssh/authorized_keys < /app/config/users/"$username"
    ...
    if [[ -d "/app/config/custom-env-vars" ]]; then
...
EOF
  ...
  chown tok:web /app/config/.env
  chmod 0750 /app/config/.env
fi
