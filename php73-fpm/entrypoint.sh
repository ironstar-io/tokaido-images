...
defaults[PHP_DEFAULT_CHARSET]="UTF-8"
...
crudini --set /etc/php/7.3/fpm/php.ini PHP max_execution_time ${resolved[PHP_MAX_EXECUTION_TIME]}
crudini --set /etc/php/7.3/fpm/php.ini PHP max_input_time ${resolved[PHP_MAX_INPUT_TIME]}
crudini --set /etc/php/7.3/fpm/php.ini PHP max_input_vars ${resolved[PHP_MAX_INPUT_VARS]}
crudini --set /etc/php/7.3/fpm/php.ini PHP memory_limit ${resolved[PHP_MEMORY_LIMIT]}
crudini --set /etc/php/7.3/fpm/php.ini PHP display_errors ${resolved[PHP_DISPLAY_ERRORS]}
crudini --set /etc/php/7.3/fpm/php.ini PHP display_startup_errors ${resolved[PHP_DISPLAY_STARTUP_ERRORS]}
crudini --set /etc/php/7.3/fpm/php.ini PHP log_errors ${resolved[PHP_LOG_ERRORS]}
crudini --set /etc/php/7.3/fpm/php.ini PHP log_errors_max_len ${resolved[PHP_LOG_ERRORS_MAX_LEN]}
crudini --set /etc/php/7.3/fpm/php.ini PHP ignore_repeated_errors ${resolved[PHP_IGNORE_REPEATED_ERRORS]}
crudini --set /etc/php/7.3/fpm/php.ini PHP ignore_repeated_source ${resolved[PHP_IGNORE_REPEATED_SOURCE]}
crudini --set /etc/php/7.3/fpm/php.ini PHP report_memleaks ${resolved[PHP_REPORT_MEMLEAKS]}
crudini --set /etc/php/7.3/fpm/php.ini PHP post_max_size ${resolved[PHP_POST_MAX_SIZE]}
crudini --set /etc/php/7.3/fpm/php.ini PHP default_charset ${resolved[PHP_DEFAULT_CHARSET]}
crudini --set /etc/php/7.3/fpm/php.ini PHP file_uploads ${resolved[PHP_FILE_UPLOADS]}
crudini --set /etc/php/7.3/fpm/php.ini PHP upload_max_filesize ${resolved[PHP_UPLOAD_MAX_FILESIZE]}
crudini --set /etc/php/7.3/fpm/php.ini PHP max_file_uploads ${resolved[PHP_MAX_FILE_UPLOADS]}
crudini --set /etc/php/7.3/fpm/php.ini PHP allow_url_fopen ${resolved[PHP_ALLOW_URL_FOPEN]}

# Worker pool settings can only be configured via environment variables, not via .tok/config.yml
crudini --set /etc/php/7.3/fpm/pool.d/www.conf www pm.max_children ${resolved[WWW_PM_MAX_CHILDREN]}
crudini --set /etc/php/7.3/fpm/pool.d/www.conf www pm.start_servers ${resolved[WWW_PM_START_SERVERS]}
crudini --set /etc/php/7.3/fpm/pool.d/www.conf www pm.min_spare_servers ${resolved[WWW_PM_MIN_SPARE_SERVERS]}
crudini --set /etc/php/7.3/fpm/pool.d/www.conf www pm.max_spare_servers ${resolved[WWW_PM_MAX_SPARE_SERVERS]}
crudini --set /etc/php/7.3/fpm/pool.d/www.conf www pm.process_idle_timeout ${resolved[WWW_PM_PROCESS_IDLE_TIMEOUT]}
...
if [ ${NEWRELIC_LICENSE_KEY+x} ]; then
  ...
  crudini --set /etc/php/7.3/fpm/conf.d/20-newrelic.ini newrelic newrelic.enabled true
  crudini --set /etc/php/7.3/fpm/conf.d/20-newrelic.ini newrelic newrelic.license "$NEWRELIC_LICENSE_KEY"
  crudini --set /etc/php/7.3/fpm/conf.d/20-newrelic.ini newrelic newrelic.logfile "/dev/stdout"
  crudini --set /etc/php/7.3/fpm/conf.d/20-newrelic.ini newrelic newrelic.appname "$NEWRELIC_APP_NAME"
  crudini --set /etc/php/7.3/fpm/conf.d/20-newrelic.ini newrelic newrelic.daemon.logfile "/dev/stdout"
else
  # ensure newrelic module is never loaded
  printf "Disabling NewRelic Support - NEWRELIC_LICENSE_KEY not set\n"
  rm /etc/php/7.3/fpm/conf.d/20-newrelic.ini 2>/dev/null || true
fi
...
if [ ${XDEBUG_REMOTE_ENABLE+x} ]; then
    ...
    crudini --set /etc/php/7.3/fpm/conf.d/20-xdebug.ini xdebug xdebug.remote_enable ${XDEBUG_REMOTE_ENABLE}
    crudini --set /etc/php/7.3/fpm/conf.d/20-xdebug.ini xdebug xdebug.remote_connect_back ${XDEBUG_REMOTE_CONNECT_BACK}
    crudini --set /etc/php/7.3/fpm/conf.d/20-xdebug.ini xdebug xdebug.remote_host ${XDEBUG_REMOTE_HOST}
    crudini --set /etc/php/7.3/fpm/conf.d/20-xdebug.ini xdebug xdebug.remote_handler ${XDEBUG_REMOTE_HANDLER}
    crudini --set /etc/php/7.3/fpm/conf.d/20-xdebug.ini xdebug xdebug.remote_port ${XDEBUG_REMOTE_PORT}
    crudini --set /etc/php/7.3/fpm/conf.d/20-xdebug.ini xdebug xdebug.remote_autostart ${XDEBUG_REMOTE_AUTOSTART}
    crudini --set /etc/php/7.3/fpm/conf.d/20-xdebug.ini xdebug xdebug.extended_info ${XDEBUG_EXTENDED_INFO}
else
    printf "Disabling XDebug support\n"
    rm /etc/php/7.3/fpm/conf.d/20-xdebug.ini 2>/dev/null || true
fi
...
printf "${GREEN}Starting PHP FPM 7.3...\n${NC}"
